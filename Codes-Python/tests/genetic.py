...
from output import print_problem_diversities, print_problem_ours, print_problem_costs, print_population
from problem.permutation_problem import permutation_problem
from tests.tsp_problem import tsp_problem
from tests.util import City, read_cities, write_cities_and_return_them, generate_cities
...
class GeneticAlgorithm:
    ...
    def selection(self):
        ...
        if self.roulette_selection:
            df = pd.DataFrame(np.array(self.ranked_population, dtype=object), columns=["index", "fitness"])
    ...
    def run(self):
        ...
        for ind in range(0, self.iterations):
            ...
            solutions = []
            costs_sum = 0
            costs = [(chromosome, Fitness(chromosome).path_cost()) for chromosome in self.population]
            for c in costs:
                costs_sum += c[1]
            ...
            print_population(solutions, str(ind), output_path=self.output_path)
            self.problem.compute_diversities(solutions, self.best_distance(), costs_sum/self.problem.number_of_variables)
...
def run_ga():
    ...
    genetic_algorithm = GeneticAlgorithm(cities=cities, iterations=1,
                                         population_size=100, elites_num=50, mutation_rate=0.008,
                                         greedy_seed=1, roulette_selection=True, plot_progress=True)
    # Good
    # genetic_algorithm = GeneticAlgorithm(cities=cities, iterations=200, population_size=1000,
    #                                      elites_num=100, mutation_rate=0.08, greedy_seed=1,
    ...
    #                                      roulette_selection=True, plot_progress=True)
    genetic_algorithm.output_path = ""
    genetic_algorithm.run()
    print(genetic_algorithm.best_distance())
    # genetic_algorithm.plot()
    # plt.show(block=True)
    problem = genetic_algorithm.problem
    now = datetime.datetime.now()
    time_name = now.strftime("%Y%m%d-%H%M%S")+'-ga'
