...
namespace Thesis_6
{
    ...
    internal class Benchmark
    {
        ...
        public static bool InitResultPath(Data data)
        {
            ...
            string path = string.Format("{0}\\{1}\\{2}, {3}, Mod {4}, PpO {5}", 
                ResultBasePath, data.Name, DateTime.Now.ToString("yyMMddHHmmss.fff"), data.Name, data.Modality, data.PopulationCount);
        ...
        public static void Print(List<DiversityResult> diversityResults, int[] diversityRatios)
        {
            ...
            string filename = string.Format("{0}\\{1}, {2}, Mod {3}, PpO {4}.csv",
               ResultPath, DateTime.Now.ToString("yyMMddHHmmss.fff"), Name, Modality, PopulationCount);
            using (StreamWriter writetext = new StreamWriter(filename))
            {
