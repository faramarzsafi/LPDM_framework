    ...
    internal class Benchmark
    {
        ...
        public Permutation[] populationGenerateSingle()
        {
            ...
            for (int i = 0; i < Optimas.Length; i++)
            {
                ...
                for (int j = 0; j < PopulationCount * 10 / 100 + 1; j++)
                    permutations[i] = new Permutation(permutations[0]);
            }
            //int duplicatesNum = 10;
            //int populationCount = PopulationCount * Optimas.Length;
            //Permutation[] population = new Permutation[populationCount * (duplicatesNum+1)];
            //Array.Copy(permutations, population, populationCount);
            //for (int i = 0; i < duplicatesNum; i++)
            //    for (int j = 0; j < populationCount; j++)
            //    {
            //        population[(i+1) * populationCount + j] = new Permutation(permutations[j]);
            //    }
            return permutations;
        }
        ...
        public static void Run(string[] args)
        {
            ...
            Benchmark benchmark = new Benchmark("T16D", 7, 1000);
