...
namespace Thesis_7
{
    ...
    internal class Benchmark
    {
        ...
        public static bool InitResultPath(Data data)
        {
            ...
            ResultFileName = string.Format("{0}, {1}, Mod {2}, PpO {3}, α {4}",
                DateTime.Now.ToString("yyMMddHHmmss.fff"), data.Name, data.Modality, data.PopulationCount, data.OurMethodAlpha * 100);
            string path = string.Format("{0}\\{1}\\{2}", ResultBasePath, data.Name, ResultFileName);
        ...
        public static void Print(List<DiversityResult> diversityResults, int[] diversityRatios)
        {
            ...
            using (StreamWriter writetext = new StreamWriter(filename))
            {
                ...
                writetext.Write("Max distance, Ratio,Osuna Enciso et al,Cheng,Salleh et al, Zhao et al");
                for (int i = 0; i < ClustersSizes.Length; i++)
                ...
                {
                    DiversityResult result = diversityResults[i];
                    writetext.Write("{0:0.000},{1:0.000},{2:0.000},{3:0.000},{4:0.000},{5:0.000}",
        ...
        public static void Run()
        {
            ...
            data.OurMethodAlpha = 0.8;
            data.ClustersSizes = new BigInteger[] { 5, 36, 72, 144, 288 };
//            data.ClustersSizes = new BigInteger[] { 18, 36, 72, 144, 288 };
